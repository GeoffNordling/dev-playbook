...
| `standards-lint` | [cards.md](/standards/standard/cards.md), [detectors.md](/standards/standard/detectors.md) | The Card Catalog's rules over `standards/` — directory layout, the question sentence, the catalog, hook-surface agreement, and no shadowing of an upstream card (consumer mode); clean by construction where no `standards/` tree is present |
