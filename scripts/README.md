...
| `cascade` | Move every governed repo's dev-playbook pin to the release head — for each repo on this machine with no ledger row at that head, probe the bump in a throwaway worktree of `origin/main`; green lands one commit on `main`, red cuts a `bump-pin-<sha12>` worktree and hands it to a headless agent that works the findings and opens a PR — and append one row per repo to the [Pin Cascade Ledger](/docs/pin-cascade.md); run by a systemd user timer every fifteen minutes, exits at once when every repo is recorded; merges nothing; `--dry-run` probes and lands nothing |
