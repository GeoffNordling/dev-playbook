...
[fact base](/worktree-cloa-viewer-tool-2-working-docs/fact-base.md#views-are-selections):
...
Standard, its card, its population, and its rules with their conditions
([Population and Rules Encoding](/doc-types/standard/encoding.md)); for
a Standard-Card, the four cells and the pointers in each
([Card Cells](/doc-types/standard-card/contract-shape.md)). A runbook
also has a control flow view, the same edges with order and condition
kept.

How these panels draw is not settled, and the kind names are not
chosen. The renderer ideas for the runbook are in
[Design](/worktree-cloa-viewer-tool-2-working-docs/design.md), and the
