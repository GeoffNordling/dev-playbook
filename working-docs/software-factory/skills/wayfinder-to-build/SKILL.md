...
docs/guides/modules.md`. Proceed only after.
...
3. **The ordering** — dependency order, with the reason for each edge. A
   dependency is any **consumed artifact**: a slice that dispatches or cites
...
[the decompose exit](~/workspace/dev-playbook/working-docs/software-factory/skills/design/references/decompose.md);
...
- **Decomposition rationale** — the slicing decision and its ordering
  ...
  provenance.
- **Standing rulings** — seeded with exactly these, which bind every child
  session (they read the epic):
...
[issue-review verdict](~/workspace/dev-playbook/working-docs/software-factory/docs/user-checkpoints.md#the-issue-review-verdict)
