...
- **Paddings a node body invites.** The [no-op test](/guides/writing-for-agents.md#pruning) cuts what the agent would do anyway. Node-shaped instances: explaining how a step serves a later phase the agent can't see ("commit so the later PR has content to carry"), and guarding a scenario the factory design settled ("a reboot won't lose the worktree"). Keep the conditions and escalations; drop the color.
