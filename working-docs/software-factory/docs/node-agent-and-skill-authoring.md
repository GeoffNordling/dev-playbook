...
Conventions for writing what runs the software factory's nodes: the typed agent definitions in `dotfiles/dot-claude/agents/` and the `phase:*` skills in `dotfiles/dot-claude/skills/`. One authoring style governs both — a definition and a skill differ in how they are launched and how they report, not in how they are written — and every rule below applies to both unless it names one. What a node *does* is bound elsewhere, and this one restates none of it: [software-factory.md](/working-docs/software-factory/docs/software-factory.md) defines the graph and the states; [factory-operations.md](/working-docs/software-factory/docs/factory-operations.md) defines the dispatch model and the [node-skill contract](/working-docs/software-factory/docs/factory-operations.md#the-node-skill-contract) — `## Read first`, the worktree, the gate, the terminal report contract and its report line; [review-contract.md](/working-docs/software-factory/docs/review-contract.md) defines everything a review does once dispatched, from its stance to the threads it leaves behind. Clauses of that node-skill contract bind a skill only: a definition never enters a worktree — its launcher sets the cwd — and it ends on the report envelope rather than a report line. What follows is the authoring *style* behind them.

Its base is the general runbook machinery, equally unrestated: [runbook-conventions.md](/standards/doc-type/runbook-conventions.md) binds the format a skill or agent definition takes, [writing-for-agents.md](/guides/writing-for-agents.md) carries the craft both answer to, and the `runbook-creator` skill is the workflow over both. What follows is what a node needs on top of them.
...
- **Paddings a node body invites.** The [no-op test](/guides/writing-for-agents.md#every-line-earns-its-load) cuts what the agent would do anyway. Node-shaped instances: explaining how a step serves a later phase the agent can't see ("commit so the later PR has content to carry"), and guarding a scenario the factory design settled ("a reboot won't lose the worktree"). Keep the conditions and escalations; drop the color.
...
- **A body does not enumerate its permissions.** They are the session's: auto mode judges each call, and subagent permissions are consciously wide ([factory-operations.md § Permissions](/working-docs/software-factory/docs/factory-operations.md#permissions)). The exception is a role boundary the harness must enforce rather than merely be asked for — a reviewer's read-only stance is stated in the body's prose *and* nailed down in frontmatter, by the form's own mechanism: a definition pins the `tools` roster it needs and the file-writing tools are simply absent from it, while a skill denies them outright with `disallowed-tools` ([runbook-conventions.md — Front matter](/standards/doc-type/runbook-conventions.md#front-matter-holds-its-kinds-vocabulary)).
