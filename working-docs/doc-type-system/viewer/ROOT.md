...
and this strand is its plan. Speculative, per
...
([Standing](/working-docs/doc-type-system/fact-base/ROOT.md#standing)).
...
- **view file** — one JSON file the server writes and the viewer shows as
  ...
  ([Contract](/working-docs/doc-type-system/viewer/contract.md)).
...
- **kind** — a registered view: a selection, a renderer, and a doc page
  under one name ([an entry](/working-docs/doc-type-system/viewer/registry.md#an-entry)).
  The set's word for it is view; the pages and the code keep kind
  until the rename lands
  ([Planned](/working-docs/doc-type-system/viewer/ROOT.md#planned)).
- **registry** — the list of kinds the tool can show
  ([Registry](/working-docs/doc-type-system/viewer/registry.md)).
...
- **state directory** — the directory outside every repo where view files
  live
  ([the state directory](/working-docs/doc-type-system/viewer/contract.md#the-state-directory)).
- **identity** — how a view file names a thing in a checkout
  ([identities](/working-docs/doc-type-system/viewer/contract.md#identities)).
- **envelope** — the fixed top-level fields every view file carries
  around its payload
  ([the envelope](/working-docs/doc-type-system/viewer/contract.md#the-envelope)).
...
- **arrangement** — what the user has open, saved per checkout
  ([the arrangement](/working-docs/doc-type-system/viewer/contract.md#the-arrangement)).
- **refresh record** — the outcome of the last refresh for one checkout
  ([the refresh record](/working-docs/doc-type-system/viewer/contract.md#the-refresh-record)).
...
[Live update](/working-docs/doc-type-system/viewer/server.md#live-update)
targets.
...
- [Contract](/working-docs/doc-type-system/viewer/contract.md) — the
  state directory, view file paths, identities, the envelope, the
  arrangement, the refresh record, and the failure rules.
- [Registry](/working-docs/doc-type-system/viewer/registry.md) — what
  a registry entry is, the kinds built, and the kinds deferred.
- [Design](/working-docs/doc-type-system/viewer/design.md) — how a
  panel is designed, and the renderer ideas recorded for the runbook
  views.
- [Viewer](/working-docs/doc-type-system/viewer/viewer.md) — the page
  in the browser: layout, tree, panels, toggle, refresh, and failure on
  screen.
- [Server](/working-docs/doc-type-system/viewer/server.md) — the
  command, the server's jobs, and live update.
- [Stack](/working-docs/doc-type-system/viewer/stack.md) — the two
...
which the fact base strand proves by simulation and then extracts
...
- **Runbook design** — a design session for the runbook views, starting
  ...
  [Design](/working-docs/doc-type-system/viewer/design.md): the
...
- **Kind becomes view** — kind and view are one thing under two
  words ([Terms](/working-docs/doc-type-system/ROOT.md#terms)):
