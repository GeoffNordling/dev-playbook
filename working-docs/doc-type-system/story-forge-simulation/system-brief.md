...
type: General-Sheet
title: System Brief
description: The doc-type system and the fact base as one reader holds them after one pass through the set — the doctrine, the four doc-types and their pseudocode, the two written forms, the encoding, the fact base and its views, the build loop, and the two ontologies a consumer repo splits into, each with the file that carries it
---
...
it names instead of the whole set again. Speculative, per
...
condition, and a why. The words are the set's
[Terms](/working-docs/doc-type-system/ROOT.md#terms), one meaning
...
([The build loop runs over every object](/working-docs/doc-type-system/fact-base/fact-base.md#the-build-loop-runs-over-every-object)).
...
the fact base root's Planned item
[Simulations by hand](/working-docs/doc-type-system/fact-base/ROOT.md#planned):
...
[Across the workspace](/working-docs/doc-type-system/fact-base/fact-base.md#across-the-workspace)
...
1. This file.
2. [Synthesis Working Root](/working-docs/doc-type-system/ROOT.md),
   for the terms.
3. [Ralph Fact Base](/working-docs/doc-type-system/fact-base/fact-base-ralph.md)
   and its JSON, for the row shape a simulation writes.
4. [Story-Forge Survey](/working-docs/doc-type-system/story-forge-simulation/story-forge-survey.md),
   for the specimen.
