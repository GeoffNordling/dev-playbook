---
type: General-Sheet
title: Contested Calls
description: The twelve step 11 rows where the reviewer disagreed with the auditor, and the decision on each
---
...
|---|---|---|
| `doc-type.steps-end-on-a-completion-criterion` | reviewer: delete, two skills end steps on actions | guide: the idea is sound and unchecked; it joins `writing-for-agents.md` |
...
| `build.lockfile-committed` | reviewer: keep because repo-lint emits the id | delete; the check goes with it |
| `distribution.a-pinned-rev` | reviewer: keep because workspace-lint emits the id | delete; the check goes with it |
