...
- `stint.py` — the headless driver: runs one stint from the principal's opening call to the yield, with the stop rules and checkpoints in the script and the step swappable between `call.mjs` and a local `claude -p`; reads the copy as plain files only, and writes the stint's record to its folder
