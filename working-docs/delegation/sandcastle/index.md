...
Sandcastle, with its experiments and code. Start at the root.

Ordering: the root, then reading order.

- [Sandcastle Working Root](/working-docs/delegation/sandcastle/ROOT.md) — The root of the sandcastle set inside delegation — the pipeline that runs an unattended stint in a sealed container, the hard bounds it runs under, its words, and its worklist
