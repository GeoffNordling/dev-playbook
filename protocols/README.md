...
The formulation lives in the protocol directory. The skill lives in its own dotfiles bundle — `dotfiles/dot-claude/skills/protocol-<name>/SKILL.md` — which stow installs into `~/.claude/skills/` so that Claude Code can invoke it. That bundle is the skill's single copy; edit it there.
