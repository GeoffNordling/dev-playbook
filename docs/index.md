...
- [Pin Cascade Ledger](/docs/pin-cascade.md) — Every run of the pin cascade, one row per governed repo — when it ran, which release head it moved the pin to, the verdict at that head, where the bump landed, and the remote branches the run saw still unmerged
