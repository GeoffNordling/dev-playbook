...
This is a current-state record. Every row cites the Decision Record that ruled
it, and the reasoning lives there.

## Verdict vocabulary

A verdict covers the skill entire: upstream prose is never mined for fragments
...
now read `reject` and say so in their notes. Material those verdicts landed
stays where it landed — the relabel retires a vocabulary.
...
The workspace takes no dependency on a tier its upstream author has not
promoted ([0016](/docs/decisions/0016-pocock-skills-sweep-2026-07.md)). Under
the whole-skill rule above, an unpromoted skill is not mined for ideas either.
...
| git-guardrails-claude-code | misc | reject | The PAT makes the guarded operations impossible | 0016, 0012 | 0012 rules against a second published hook |
