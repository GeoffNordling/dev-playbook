...
- **spec-tools** (`~/workspace/spec-tools/`) — the workspace's
  ...
  [Decision Record 0001](0001-adopt-matt-pocock-conventions.md) rejected for Matt
  Pocock's `/to-prd`. The reasoning applies identically.
- **Matt Pocock's conventions** (adopted in
  [Decision Record 0001](0001-adopt-matt-pocock-conventions.md)) — `/tdd`,
  ...
  competes directly with `/tdd` (canonicalized in Decision Record 0001), and
...
from Decision Record 0001 into an explicit workspace principle:
...
Decision Record 0001 implicitly applied this rule — Pocock's skills came with
...
|---|---|
| Adopt Superpowers wholesale | Retires the spec-tools SDD layer and the Pocock `/tdd` canonicalization from Decision Record 0001. The case for that exchange is not made by anything in Superpowers. |
...
| Adopt the SessionStart hook *technique* (not Superpowers content) for workspace-wide priming | Catalogued as a deferred follow-up. No concrete priming need has surfaced; if one does, that should be its own Decision Record. |
...
- No changes to `dotfiles/.agents/.skill-lock.json`. No new third-party
  skills installed.
- Decision Record 0003 stands as the workspace's documented position on Superpowers;
...
- New workspace principle ("Adopt third-party skills only when their
  ...
  [Decision Record 0004](0004-remove-pocock-direct-dependency.md), and again to two
  newly-published Pocock skills; see
  [Decision Record 0006](0006-harvest-pocock-prototype-and-handoff.md).
...
- The SDD-extension bet (from
  ...
  [Decision Record 0001](0001-adopt-matt-pocock-conventions.md)) remains testable.
  ...
  current-state coding agents, this Decision Record's wholesale rejection should be
  revisited.
