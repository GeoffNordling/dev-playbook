...
- **Run the project's lint, format, and typecheck commands before presenting your changes** (check `CLAUDE.md` or `Makefile` for the exact commands). When a check fails, self-correct rather than accumulating errors across tasks. When running ruff, pass `--ignore F401` to suppress unused-import errors; the design phase leaves commented-out old implementations in stubs, which can make imports appear unused until the green agent writes the real code. Do not add `noqa` comments to suppress these; the flag keeps the code clean.
