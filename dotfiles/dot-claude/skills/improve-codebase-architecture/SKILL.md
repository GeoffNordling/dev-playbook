...
- {Read the [Design and Testing guide](~/workspace/dev-playbook/guides/design-and-testing.md#a-module-is-shaped-from-the-outside-in) for the architecture vocabulary and its principles, loaded first; `module`, `interface`, `depth`, `seam`, `adapter`, `leverage`, `locality` — the deletion test, "the interface is the test surface," "one adapter = hypothetical seam, two = real"}. Use these terms exactly in every suggestion — don't drift into "component," "service," "API," or "boundary."
