---
name: frozen-build
description: Carries out a direct-mode issue against its brief — the copy frozen for the factory rebuild. Use when the frozen issue overwatch launches the `build` node.
...
- **Rework re-entry.** Follow [PR feedback](~/.claude/skills/frozen-issue-overwatch/references/pr-feedback.md) — read it now if `gh pr view` finds a PR, and skip it otherwise. The brief is the contract it names: where a finding merely disagrees with the brief, the brief wins; where it shows the brief contradicting reality, that is a deviation — run the [deviation contract](~/.claude/skills/frozen-issue-overwatch/references/deviation-contract.md)'s limiters like any other contradiction.
...
4. Emit the terminal line, then stop:
   ```
   DONE: <repo>#<issue> · phase: build · commit <sha> · check green · pushed
   ```
