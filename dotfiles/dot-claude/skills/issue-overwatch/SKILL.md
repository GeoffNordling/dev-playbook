...
- **approve** — record the deferrals first: for each finding the user ruled real-but-not-this-issue, mint its tracker stub (`gh issue create` at `phase:intake`) and record the ruling as one PR comment naming the stubs — the approve-time regeneration lifts them into `## Deferred` from the PR record. Then regenerate the merge message from the whole PR record, per [the two owners](~/workspace/dev-playbook/software-factory/factory-operations.md#the-two-owners): write it to `/tmp/pr-body-<N>.md` with the Write tool, not a heredoc — the sandbox bounds shell commands, not the file tools, so Bash gets `Read-only file system` there and the Write tool doesn't — and pass it with `gh pr edit --body-file`. Then carry the issue to the user's final read and merge.
