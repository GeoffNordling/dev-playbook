...
**The user is in the terminal and nowhere else.** Every question, verdict request, and escalation below is briefed per the [briefing rule](~/workspace/dev-playbook/software-factory/checkpoints.md#the-briefing-rule).
...
- [checkpoints.md](~/workspace/dev-playbook/software-factory/checkpoints.md) — every point you stop at, and what you owe the user there.

Then report: `READ: software-factory.md, factory-operations.md, checkpoints.md`. Proceed only after.
...
- **ESCALATE** — bubble it to the user per the [escalation rule](~/workspace/dev-playbook/software-factory/checkpoints.md#escalation), and stop.
...
The audit subagents post findings and terminate; the verdict interview is yours. Read **every** [comment surface](~/workspace/dev-playbook/software-factory/pr-feedback.md#the-comment-surfaces) on the PR, brief the user per [pause 1](~/workspace/dev-playbook/software-factory/checkpoints.md#pause-1-the-review-verdict), answer their questions, help them weigh, and act only on an explicit verdict.
...
End your turn wherever the user must act or decide, per [turn boundaries](~/workspace/dev-playbook/software-factory/checkpoints.md#turn-boundaries). Where the ending carries the user's command, state it once, paste-safe, one line:
