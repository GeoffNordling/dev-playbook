...
description: Executes one issue's traverse through the software factory — runs or delegates each factory node, writes the labels, and stops wherever the user must act or decide. Use when the user launches it against a single issue number from Agent view.
