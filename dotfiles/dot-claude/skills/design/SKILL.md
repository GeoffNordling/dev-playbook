...
1. **Run the issue-review beat** — for the leaf this session designed, fresh or decomposition-minted. The two lenses are **your tools, not the user's**. Dispatch both in one message, as fresh-context subagents: one invokes `/issue-review-claims <issue>`, the other `/issue-review-simulation <issue>`, each pinned to the model its skill file names and reading only the issue and the repo. Merge and deduplicate their findings, then apply or demote each on your judgment, editing until it is a brief you would hand an autonomous builder. Never stop to have the user rule. Post the **verdict-record comment**, then hand them the issue URL, what the lenses caught, what you changed, and what is unresolved ([the issue-review verdict](~/workspace/dev-playbook/software-factory/user-checkpoints.md#the-issue-review-verdict)). They may always skip or cut short the beat.
2. **Move the phase** — single leaf only, and only on their approval: `gh issue edit <issue> --remove-label "phase:design" --add-label "phase:build"`. Asked for changes, apply and re-present. Judged not ready, it stays at `phase:design`, the verdict record its next work order; only this move is skipped, so the remaining steps run either way. A decomposed issue shed its phase at §7 and stays an epic; each child crosses on its own approval here.
