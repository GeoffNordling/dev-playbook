...
1. **Run the issue-review beat** — for the leaf this session designed, whether it arrived here fresh or as a child a decomposition minted. Dispatch both lenses in one message, as fresh-context subagents: one invokes `/issue-review-claims <issue>`, the other `/issue-review-simulation <issue>`, each pinned to the model its skill file names; they read only the issue and the repo and return findings raw. Synthesize the **consolidated disposition list** and take the user verdict per [the issue-review verdict](~/workspace/dev-playbook/software-factory/user-checkpoints.md#the-issue-review-verdict): *pass* — apply or demote per the ruled dispositions, post the **verdict-record comment**, and step 2's label move follows; *back to design* — the issue stays at `phase:design` with the verdict record as the next session's work order, and only step 2's label move is skipped: the exit still closes, so the remaining steps run either way. The user may always skip the beat, cut it short, or advance anyway.
2. **Move the phase** — single leaf only, on step 1's *pass*: `gh issue edit <issue> --remove-label "phase:design" --add-label "phase:build"`, and the issue crosses into the factory. A decomposed issue shed its phase at §7 and stays an epic; its children carry the work, and each crosses on its own pass here.
