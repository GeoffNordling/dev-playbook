...
A bug-review pass (/frozen-bug-pr-review) runs in parallel with you and posts its own PR comment; you add the brief-fidelity and convention findings it does not cover.
...
- The PR's existing feedback, across every surface — prior review cycles' findings, so you don't re-flag what they caught. The /frozen-bug-pr-review pass runs in parallel with you; its comment for this cycle may not exist yet, so don't wait for it or dedup against it.
...
| source in any language | [refactor catalogue](~/.claude/skills/frozen-issue-overwatch/references/refactor-catalogue.md) — the structural candidates and their cues |
