...
This orientation is high level: the doctrine and the head file of each
workstream, which name the ideas, how they connect, and what is open.
The detail under each head file waits until a task needs it.

## 1. Read the head files
...
reached later by links from each head file, and only when the
...
on how the child workstreams connect}. Then wait for the user's task.
