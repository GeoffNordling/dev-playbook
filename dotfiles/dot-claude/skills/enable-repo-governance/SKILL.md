...
`git rev-parse --show-toplevel` names it. {If its working tree is not clean and on `main`, {report that the tree is
not clean} and stop}; the adoption diff must carry adoption and nothing
else. {If the repo is already in workspace-lint's `GOVERNED` roster, {report
that it is already governed} and stop}: that repo's problem is pin drift or
findings, which is /update-standards-pin territory.
...
{Run [bootstrap-labels](~/workspace/dev-playbook/scripts/bootstrap-labels)}.
...
Enrollment follows the adoption onto `main`: {Write the target's name into
the `GOVERNED` roster} in
[workspace_lint.py](~/workspace/dev-playbook/src/dev_playbook/workspace_lint.py),
then {Run [workspace-lint](~/workspace/dev-playbook/scripts/workspace-lint)}
to confirm the newly enrolled repo reports clean. {Commit that dev-playbook
edit separately, one line}:
...
pin sits on an unmerged branch reports a `build.pin` finding against `main`.
