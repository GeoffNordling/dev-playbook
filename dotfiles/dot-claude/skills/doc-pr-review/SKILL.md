...
|---|---|
| skills | [skill conventions](~/workspace/dev-playbook/standards/claude-code/skill-conventions.md) — the binding format, plus /writing-for-agents for the craft every skill answers to; and for a `phase:*` skill, [node-skill authoring](~/workspace/dev-playbook/software-factory/node-skill-authoring.md) on top |
