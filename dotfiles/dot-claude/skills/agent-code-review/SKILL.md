...
If the change touches a Python sub-project, run its gate — `make -C <subproject> check` (or `make check` when the `Makefile` is at the repo root) — the full chain: format-check, lint, typecheck, test. Green: proceed to the audit. Red: build opened a PR over a red tree — escalate (§6) rather than review broken work. A change touching no Python sub-project has no `make` gate; proceed to the audit.
