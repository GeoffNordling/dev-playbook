...
— is [software-factory.md](~/.claude/skills/frozen-issue-overwatch/references/software-factory.md); every point
where it stops for the user is
[user-checkpoints.md](~/.claude/skills/frozen-issue-overwatch/references/user-checkpoints.md).
...
[definition](~/.claude/skills/frozen-issue-overwatch/references/software-factory.md#the-definition-region) — or on
...
|---|---|---|
| `build` | `/frozen-build` | AFK. |
| `pr_review` | `/frozen-open-pr` first, always, then the [track](#track-rules) skills | AFK per skill, then the user's verdict on the whole stop ([pause 1](~/.claude/skills/frozen-issue-overwatch/references/user-checkpoints.md#pause-1-the-review-verdict)). |
...
[user-checkpoints.md](~/.claude/skills/frozen-issue-overwatch/references/user-checkpoints.md#escalation).
...
lives in [node-skill-authoring.md](~/.claude/skills/frozen-issue-overwatch/references/node-skill-authoring.md).
...
- **Body** — three mandatory sections, each checkable by absence:
  ...
  - `## Deviation ledger` — the deviation entries of the
    [deviation contract](~/.claude/skills/frozen-issue-overwatch/references/deviation-contract.md#the-deviation-ledger),
...
`/frozen-open-pr` authors it when it creates the PR — lifting the build session's
recorded entries into `## Deviation ledger` per the
[contract's hand-off](~/.claude/skills/frozen-issue-overwatch/references/deviation-contract.md#the-deviation-ledger)
...
- **Code track.** `/frozen-bug-pr-review` posts its bug findings; `/frozen-code-pr-review` adds
  the fidelity and convention findings it does not cover.
- **Doc track.** `/frozen-doc-pr-review` audits the diff's documentation.
- **Lockdown re-review.** From the third cycle on, each live track runs its
  fidelity-and-convention review alone — `/frozen-code-pr-review` on the code track,
  `/frozen-doc-pr-review` on the doc track — and `/frozen-bug-pr-review` stands down: a
...
[pause 1](~/.claude/skills/frozen-issue-overwatch/references/user-checkpoints.md#pause-1-the-review-verdict). A
