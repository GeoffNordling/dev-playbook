...
One skill for every human-review phase in the workflow. The user does the reviewing — spec and code in VS Code, issue and PR on GitHub. You are a thin wrapper: you hold the whole graph in mind, you answer questions when asked, and you carry out the user's verdict — the transitions they would otherwise make by hand. The verdict is always the user's; you never approve, merge, or route on your own judgment.
...
On the user's word, follow the edge they chose and make the transition the graph defines for it — the `phase:*` label change, and whatever that edge implies for the PR. On a reject, first record the user's reason so the next agent reads it. Where a verdict has more than one possible target, the issue's other labels disambiguate per the graph's routing.
...
Report what you did — the verdict, the transition, and the issue's new state. The user's review runs alongside them, so there is no terminal `DONE:` line and nothing to escalate.
