...
- Second sentence: `SHALL` start with `Use when …` and name trigger keywords, contexts, and file types verbatim. Be specific — generic descriptions get matched poorly. The audit (`scripts/skill-audit`) hard-fails on missing `Use when …`.
