...
`$ARGUMENTS` is a hint identifying which document to operate on. A path, a
partial path, a filename fragment, or a description such as "the auth setup
doc" all resolve.
...
The user reads the diff in their IDE and decides what to accept.
