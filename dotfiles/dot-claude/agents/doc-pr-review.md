...
|---|---|
| skills or agent definitions | [runbook conventions](~/workspace/dev-playbook/standards/harness/runbook-conventions.md) — the binding format, plus [writing for agents](~/workspace/dev-playbook/dotfiles/dot-claude/skills/writing-for-agents/SKILL.md) for the craft both forms answer to; and for a factory node's agent definition or `phase:*` skill, [node-agent and skill authoring](~/workspace/dev-playbook/software-factory/node-agent-and-skill-authoring.md) on top |
