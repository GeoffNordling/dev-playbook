...
main checkout or issue worktree — never the `~/.claude/` symlink. Pick the
checkout per same-repo resolution: from a dev-playbook issue worktree, edit
that worktree's `dotfiles/dot-claude/` so the change rides the issue branch,
