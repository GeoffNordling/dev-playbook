"""The fixed judge configuration: the verbatim prompt and the output schema.

Both are importable module constants because the downstream layer must run the
judge under *exactly* the prompt and schema that went into the fingerprint;
otherwise the fingerprint would not describe what actually ran. Editing either
one re-keys (and so re-runs) every judgment everywhere — by design.
"""
