...
def _ensure_fixture_importable():
    # Ensure the tests/ dir is on sys.path so 'iface_fixture' resolves.
...
class TestQualifiedName:
    def test_simple(self):
        assert qualified_name("foo.bar(x: int) -> None") == "foo.bar"

    def test_method(self):
        assert qualified_name("a.b.C.method(self) -> None") == "a.b.C.method"

    def test_no_paren(self):
        assert qualified_name("not a signature") == ""


class TestResolveSymbol:
    def test_module_function(self):
        obj = resolve_symbol("iface_fixture.parse_session")
        assert obj is fix.parse_session

    def test_class_method(self):
        obj = resolve_symbol("iface_fixture.Session.run")
        assert obj is fix.Session.run

    def test_missing_symbol_returns_none(self):
        assert resolve_symbol("iface_fixture.no_such_thing") is None

    def test_missing_module_returns_none(self):
        assert resolve_symbol("no.such.module.fn") is None
...
class TestRenderSignature:
    def test_simple_function(self):
        rendered = render_signature("iface_fixture.parse_session", fix.parse_session)
        assert rendered == "iface_fixture.parse_session(path: pathlib.Path) -> str"

    def test_default_marked_with_ellipsis(self):
        rendered = render_signature("iface_fixture.with_default", fix.with_default)
        assert rendered == "iface_fixture.with_default(value: int = ...) -> int"

    def test_optional_uses_modern_union(self):
        rendered = render_signature(
            "iface_fixture.with_optional", fix.with_optional
        )
        assert rendered == "iface_fixture.with_optional(value: int | None = ...) -> int"

    def test_list_annotation(self):
        rendered = render_signature("iface_fixture.with_list", fix.with_list)
        assert rendered == "iface_fixture.with_list(items: list[int]) -> int"

    def test_kwonly_marker(self):
        rendered = render_signature("iface_fixture.with_kwonly", fix.with_kwonly)
        assert rendered == "iface_fixture.with_kwonly(*, flag: bool) -> bool"

    def test_var_args(self):
        rendered = render_signature(
            "iface_fixture.with_var_args", fix.with_var_args
        )
        assert rendered == "iface_fixture.with_var_args(*args: int, **kwargs: str) -> None"

    def test_method_includes_self(self):
        rendered = render_signature("iface_fixture.Session.run", fix.Session.run)
        assert rendered == "iface_fixture.Session.run(self) -> None"
...
class TestValidateInterfaces:
    def test_match_no_findings(self):
        item = _dsn("iface_fixture.parse_session(path: pathlib.Path) -> str")
        assert validate_interfaces([item]) == []

    def test_mismatch_emits_finding(self):
        item = _dsn("iface_fixture.parse_session(path: str) -> str")
    ...
    def test_missing_symbol_emits_finding(self):
        item = _dsn("iface_fixture.no_such_function(x: int) -> None")
        findings = validate_interfaces([item])
        assert len(findings) == 1
    ...
    def test_non_dsn_skipped(self):
        item = SpecItem(
            ...
            source_line=1,
            interfaces=["iface_fixture.no_such_thing(x: int) -> None"],
        )
        assert validate_interfaces([item]) == []
