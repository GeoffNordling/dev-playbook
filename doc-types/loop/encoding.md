...
description: The layer below the shape — how a Loop's file writes its graph, its acts, its checks, and its yields for loopgen, and where the file sits
---
...
takes so `scripts/loopgen` reads every act, check, yield, and condition
...
the pointers and the conditions in full. loopgen checks that the two
agree; it never interprets either.
