...
description: The head file of the doc-type system child workstream — the language of doc-types, verbs, and predicates, its principles and terms, the open questions, and the completed refactor from the Standard's new shape through every rule settled
---
...
for use outside the workstream.
...
- **Both forms.** A target state is written twice: as a reference
  ...
  [Terms](/workstreams/doc-type-system/WORKSTREAM.md#terms), one
  meaning each.
...
- **Step 11 wave 3, 2026-09-22.** The repo changes, by hand in the
  ...
  terms and the `WORKSTREAM.md` link tree, both duplicating what exists;
