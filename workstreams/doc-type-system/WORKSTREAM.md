---
type: Workstream
title: Synthesis Workstream
description: The head file of the unified workstream — five child workstreams and their plan dependencies, the terms every child workstream uses, the open questions that cross child workstreams, and the run order
---

# Synthesis Workstream

This workstream is speculative: every member writes a guess as a guess, and
every member inherits that voice. It unifies the workstreams of two
branches, `worktree-loop-document-type` and
`worktree-cloa-viewer-tool-2`, which describe one theory. The workstream is
five child workstreams, each with its own head file and its own worklist
([A worklist item opens with its bold name](/standards/doc-type/workstream-conventions.md#a-worklist-item-opens-with-its-bold-name)).
This head file holds only what crosses child workstreams.

## Goal

The workstream builds a typed property graph compiled from declared
...
([Precedent](/workstreams/doc-type-system/fact-base/precedent.md)).

- **Doc-type system**, the language: doc-types, verbs, rules as
  predicates, encodings as grammar. Head file:
...
- **Loop and Workstream**, what drives and what is driven: Loop and the
  new Workstream doc-type, designed together. Head file:
...
- **Fact base**, the compiled object: one deterministic object of nodes
  and edges per checkout, every view a selection from it. Head file:
...
- **Viewer**, a selection on screen: cloa-viewer, the local visual IDE
  that draws registered views of the fact base. Head file:
  [CLOA Viewer](/workstreams/doc-type-system/viewer/WORKSTREAM.md).
...
```
                         WORKSTREAM.md#terms
            one meaning per word, every child workstream
                                  │
  ┌───────────────────────────────┴────────────────────────────────┐
...
The words of the workstream, one meaning each, used in these senses by
every child workstream. This is the conceptual model, what domain-driven design calls
...
one child workstream coins for itself is in that child workstream's
head file. The entries are
...
- **Node** — one thing with an identity, a node type, a provenance, and
  ...
  ([Contract](/workstreams/doc-type-system/viewer/contract.md#identities));
