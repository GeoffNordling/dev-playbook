...
description: The head file of the fact base child workstream — the one deterministic object under every view, its standing over the viewer, its principle and terms, the worklist from hand simulations through extractors, declared data, and a stamped findings artifact, and what is done
---

# Fact Base Workstream

The child workstream that holds the compiled object: one deterministic object of
...
[Deterministic Separation](/workstreams/doc-type-system/fact-base/deterministic-separation.md)
...
with [Fact Base](/workstreams/doc-type-system/fact-base/fact-base.md),
that page has priority. It defines no word: the words the child workstream
shares are the synthesis workstream's, and its own are below.
...
defined in the synthesis workstream's
[Terms](/workstreams/doc-type-system/WORKSTREAM.md#terms). The
child workstream's own:
...
- **Simulations by hand.** Enumerate the use cases, the questions a
  ...
  ([Open questions](/workstreams/doc-type-system/fact-base/fact-base.md#open-questions)).
...
- **Extractors.** `chaingen` and `rulegen`, deleted by the doc-type
  ...
  ([Planned](/workstreams/doc-type-system/loop-and-workstream/WORKSTREAM.md#planned));
