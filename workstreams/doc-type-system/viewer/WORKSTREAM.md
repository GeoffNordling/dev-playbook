...
- [Contract](/workstreams/doc-type-system/viewer/contract.md) — the
  state directory, view file paths, identities, the envelope, the
  arrangement, the refresh record, and the failure rules.
- [Registry](/workstreams/doc-type-system/viewer/registry.md) — what
  a registry entry is, the kinds built, and the kinds deferred.
- [Design](/workstreams/doc-type-system/viewer/design.md) — how a
  panel is designed, and the renderer ideas recorded for the runbook
  views.
- [Viewer](/workstreams/doc-type-system/viewer/viewer.md) — the page
  in the browser: layout, tree, panels, toggle, refresh, and failure on
  screen.
- [Server](/workstreams/doc-type-system/viewer/server.md) — the
  command, the server's jobs, and live update.
- [Stack](/workstreams/doc-type-system/viewer/stack.md) — the two
  languages, the schema bridge, the package shape, and the checks.

## Terms

- **fact base**, **view**, **extractor** — the synthesis workstream's words
...
- **view file** — one JSON file the server writes and the viewer shows as
  ...
  ([Contract](/workstreams/doc-type-system/viewer/contract.md)).
...
- **kind** — a registered view: a selection, a renderer, and a doc page
  under one name ([an entry](/workstreams/doc-type-system/viewer/registry.md#an-entry)).
  The synthesis workstream's word for it is view; the pages and the code keep kind
  until the rename lands
  ([Planned](/workstreams/doc-type-system/viewer/WORKSTREAM.md#planned)).
- **registry** — the list of kinds the tool can show
  ([Registry](/workstreams/doc-type-system/viewer/registry.md)).
...
- **state directory** — the directory outside every repo where view files
  live
  ([the state directory](/workstreams/doc-type-system/viewer/contract.md#the-state-directory)).
- **identity** — how a view file names a thing in a checkout
  ([identities](/workstreams/doc-type-system/viewer/contract.md#identities)).
- **envelope** — the fixed top-level fields every view file carries
  around its payload
  ([the envelope](/workstreams/doc-type-system/viewer/contract.md#the-envelope)).
...
- **arrangement** — what the user has open, saved per checkout
  ([the arrangement](/workstreams/doc-type-system/viewer/contract.md#the-arrangement)).
- **refresh record** — the outcome of the last refresh for one checkout
  ([the refresh record](/workstreams/doc-type-system/viewer/contract.md#the-refresh-record)).
...
([index-tree](/workstreams/doc-type-system/viewer/registry.md#index-tree)),
...
[Live update](/workstreams/doc-type-system/viewer/server.md#live-update)
targets.
...
- **The objects are already defined; the viewer only draws them.** A
  ...
  ([Fact Base](/workstreams/doc-type-system/fact-base/fact-base.md#the-objects-are-already-defined)).
...
- **Runbook design** — a design session for the runbook views, starting
  ...
  [Design](/workstreams/doc-type-system/viewer/design.md): the
...
- **CLOA kinds** — the registry entries for the
  [CLOA views](/workstreams/doc-type-system/viewer/registry.md#cloa-views):
...
- **Kind becomes view** — kind and view are one thing under two
  words ([Terms](/workstreams/doc-type-system/WORKSTREAM.md#terms)):
