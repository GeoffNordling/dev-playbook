...
type: Survey
title: Sandcastle
description: What Sandcastle offers the delegation workstream — the driver primitives it supplies, what it leaves to the user, and what it does not cover
---
...
[the delegation workflow](/workstreams/delegation/WORKSTREAM.md)
...
inside a program the user writes. The consequence for this workstream is that
...
Its primitives map onto terms this workstream already has.
...
exits; it cannot stop and ask a question. For this workstream that is close to a
feature rather than a gap: the checkpoint belongs to the stint's own
driver, which calls `run()` once per iteration
([Settled](/workstreams/delegation/WORKSTREAM.md#settled)), and the exit is
...
a container and merges the commits back out. This workstream does want every
...
path. [The five problems](/workstreams/delegation/sandcastle/experiment-log.md#the-five-problems)
...
[The Sandcastle Pipeline](/workstreams/delegation/sandcastle/pipeline.md). The tool also offers
a no-sandbox provider that runs the agent directly on the host; this workstream
does not use it.
...
experiment two
([Experiment two](/workstreams/delegation/sandcastle/experiment-log.md#experiment-two-sandcastle-against-a-copy)).
