...
[Sandcastle pipeline](/workstreams/delegation/sandcastle/pipeline.md) was
found. Each entry is closed; what it settled that still holds lives in the
pipeline and in the
[head file](/workstreams/delegation/sandcastle/WORKSTREAM.md#constraints). The starting
...
| **Relabel** | Podman's SELinux option relabels the mounted host files ([Constraints](/workstreams/delegation/sandcastle/WORKSTREAM.md#constraints)) | Experiment two |
...
- The round trip as it still runs is the Close step of
  [the pipeline](/workstreams/delegation/sandcastle/pipeline.md#the-run-start-to-finish).
...
so the change would reach beyond this workstream. B: a plug-in of our own that
...
- The run needed two dev-playbook changes, made only in the config copy:
  ...
  [`rig/`](/workstreams/delegation/sandcastle/rig/index.md).
...
Re-run on 2026-09-24, after the workstream moved under delegation: every check
passed again.
...
- So the guess in the head file holds: the copy opens once per stint, and each
