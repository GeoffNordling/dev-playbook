---
type: Workstream
title: Delegation Workstream
description: The head file of the delegation workstream — how the user hands work to agents and takes it back, attended or unattended, the shape and words of that workflow, what is settled, what is open, and the worklist
---

# Delegation Workstream

This workstream is speculative: every member writes a guess as a guess, and every
...
- **Workstream** — one line of work: an intent and its history. It holds
  ...
  directory that may hold documentation sets; this workstream is one.
...
- **Stint** — a bounded spend of effort advancing one workstream, with a
  ...
  it, as in a session like the one that wrote this workstream: the most effective
...
- **A workstream picks its headings from one menu.** The user and the
  ...
  Acronyms are a workstream's buckets today; Done when,
...
- **The doc-types have one home.** Workstream and Loop are designed
  together in
  [Loop and Workstream](/workstreams/doc-type-system/loop-and-workstream/WORKSTREAM.md), a child workstream
  of the doc-type system workstream; this workstream holds only the workflow. Decided
  2026-09-24.
