# workstreams/system/see/fact-base/ — index

The fact base child workstream: the one deterministic object under every view,
with the ontology inside it.
Start at the head file.
...
- [Ralph Fact Base](/workstreams/system/see/fact-base/fact-base-ralph.md) — The definitions behind the simulated fact base for the Ralph loop subsystem — its terms, the extractors that yield each fact, the views selected from it, and the facts no extractor reaches
- [Deterministic Separation](/workstreams/system/see/fact-base/deterministic-separation.md) — Deterministic separation of a stochastic chain, one predicate at a time — a verifier as a containment boundary, a predicate's tail as the stochastic steps after its last check, and the tail as a query over the fact base
