...
simulates is
[Fact Base](/workstreams/system/see/fact-base/fact-base.md). This page holds
...
and view in the repo's [CONTEXT.md](/CONTEXT.md), receipt in See's
...
[envelope](/workstreams/system/see/viewer/contract.md#the-envelope)
...
| mdlink | Every markdown link `[text](target)` is a links-to edge to the resolved path. `~/.claude/` resolves to `dotfiles/dot-claude/`; `~/workspace/dev-playbook/` resolves to the checkout root. | yes, the reference checks in `checks/knowledge_organization.py` |
...
3. **The iteration agent.** Its behavior is a prompt string inside
   ...
   [Fact Base](/workstreams/system/see/fact-base/fact-base.md#code-carries-no-prose):
