...
- Before changing the pre-commit hooks (the `tools/bin/` scripts or `.pre-commit-hooks.yaml`), read the Pre-commit section of [build-conventions.md](/standards/build-conventions.md) — it explains the hook-repo model and why consumer repos then need their pinned `rev` bumped.
