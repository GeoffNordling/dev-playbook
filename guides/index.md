...
- [Writing a Check](/guides/writing-a-check.md) — How a check that decides one rule is written — in dev-playbook a function registered under the rule's id, with its test, and in a consumer repo the same shape behind the repo's own hook; read before writing a check
