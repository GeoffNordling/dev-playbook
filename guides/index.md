...
- [Adopting a Repo-Scoped Standard](/guides/consuming.md) — The consumer-repo recipe for a first repo-scoped standard — grow the standards/ tree, write and publish its checks as one hook, mirror it, and gate it
...
- [Writing a Detector](/guides/writing-a-detector.md) — How a check that decides one rule is written — in dev-playbook a function registered under the rule's id, with its test, and in a consumer repo the same shape behind the repo's own hook; read before writing a check
