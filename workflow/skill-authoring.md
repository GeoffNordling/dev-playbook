...
Conventions for writing the workflow node-skills — the `phase:*` skills in `dotfiles/dot-claude/skills/`. [workflow.md](/workflow/workflow.md) defines the graph, the dispatch model, and the Node-skill contract (`## Read first`, the terminal report contract, escalation triggers); this captures the authoring *style* behind them.

## Voice

The body is read by an agent running one node who does not know the workflow model.

- **Plain behavioral language.** No `AFK`, `HITL`, `node`, `edge`, "overwatch", or "Agent view" in the body — that vocabulary lives here and in `workflow.md`, not in front of the executing agent. The exception is a skill whose subject *is* the workflow itself: it reads `workflow.md` up front and navigates by node, edge, and state, because that is its job.
...
- **Review nodes report, they don't fix.** A node that reviews another node's output stays read-only on it — the skill's prose forbids touching the work under review (the guarantee is prompt-level; see [workflow.md § Permissions](/workflow/workflow.md#permissions)). Defects route back to the authoring node through the human's rework verdict, not the reviewer's hand.
...
- **Don't audit what a deterministic gate already enforces.** An agent reviewer's value is the judgement a check can't automate — verifier honesty, scope, design — not re-confirming what the gate already proves (items discharged, the tree builds). Don't narrate those guarantees either; spend the review on what the machine can't see.
...
- **Permissions are the session's, not the skill's.** Auto mode judges each call, and subagent permissions are consciously wide (see [workflow.md § Permissions](/workflow/workflow.md#permissions)) — a skill enumerates no allow-list or deny-list; a role boundary like a reviewer's read-only stance is stated in the skill's prose.
- **An AFK skill escalates, it doesn't ask.** It runs in a subagent with no human attached, so when it needs a human call it ends with `ESCALATE:` — the issue overwatch bubbles it up. A HITL skill is the reverse: the human is engaged, so it interviews freely via `AskUserQuestion` or plain terminal prompts and gates on the answers.
...
- **Close with the standard report line.** One ` · `-delimited line: the handle `<repo>#<N>` · `phase: <node>`. A committing node appends `commit <sha> · check green · unpushed`; a node whose real output landed on GitHub appends a pointer only (`findings on PR #<n>`, `brief in issue`), never a re-paste — the terminal points, GitHub holds the detail. An AFK skill prefixes its terminal token per the subagent return contract: the final message begins at character one with exactly `DONE: <one-line outcome>` or `ESCALATE: <one-line reason>`, any detail below the line — semantics in [workflow.md § Dispatch](/workflow/workflow.md#dispatch), where a non-matching final message reads as ESCALATE. An `ESCALATE:` carries a prose reason, the only record of the block. HITL omits the token and closes with the same line.
