...
- **Leave a deferred check out of the review entirely.** Where a check is deliberately settled once at the end of the traverse rather than at each node — the semantic judgment cache is the one in play ([software-factory.md § Node-skill contract](/software-factory/software-factory.md#node-skill-contract)) — a review skill neither arms it nor names it. Its declarations are outside the reviewer's jurisdiction and its red state mid-traverse is expected, so a finding against it is dismissed on arrival.
