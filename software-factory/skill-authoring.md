...
- **A fan-out is capped by its published manifest.** A skill that dispatches workers in waves publishes the next wave's roster — a one-line mission per worker — together with the total count, before any of that wave launches. The count is a ceiling: mid-wave the skill may not exceed it, and a new idea queues to the next wave boundary. Running dry fails loud — stop at an early checkpoint and say so, rather than quietly topping the wave up. The one mid-wave interrupt is a discovery that invalidates the run's premise or poses active risk; everything else waits for the boundary.
