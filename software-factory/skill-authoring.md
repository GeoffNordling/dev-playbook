...
- **Review nodes report, they don't fix.** A node that reviews another node's output stays read-only on it — the skill's `disallowed-tools` frontmatter denies it the Edit/Write family, prose alone being too weak to carry the guarantee (see [software-factory.md § Permissions](/software-factory/software-factory.md#permissions)). Defects route back to the authoring node through the human's rework verdict, not the reviewer's hand.
...
- **Permissions are the session's, not the skill's.** Auto mode judges each call, and subagent permissions are consciously wide (see [software-factory.md § Permissions](/software-factory/software-factory.md#permissions)) — a skill enumerates permissions only where a role boundary needs a guarantee prose can't carry, as a reviewer's read-only stance does: a `disallowed-tools` deny on the Edit/Write family, paired with the narrow `allowed-tools` carve-out that staging a finding under `/tmp` needs.
