...
- **Review nodes report, they don't fix.** A node that reviews another node's output stays read-only on it — the skill's prose forbids touching the work under review (the guarantee is prompt-level; see [factory-operations.md § Permissions](/software-factory/factory-operations.md#permissions)). Defects route back to the authoring node through the human's rework verdict, not the reviewer's hand.
...
- **Permissions are the session's, not the skill's.** Auto mode judges each call, and subagent permissions are consciously wide (see [factory-operations.md § Permissions](/software-factory/factory-operations.md#permissions)) — a skill enumerates no allow-list or deny-list; a role boundary like a reviewer's read-only stance is stated in the skill's prose.
