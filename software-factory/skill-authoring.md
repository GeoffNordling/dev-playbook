...
Conventions for writing the software factory node-skills — the `phase:*` skills in `dotfiles/dot-claude/skills/`. [software-factory.md](/software-factory/software-factory.md) defines the graph and the states; [factory-operations.md](/software-factory/factory-operations.md) defines the dispatch model and the node-skill contract (`## Read first`, the terminal report contract, escalation triggers); this captures the authoring *style* behind them.

Its base is the general skill machinery, restated nowhere below: [skill-conventions.md](/standards/claude-code/skill-conventions.md) binds the bundle format, [skill-writing.md](/standards/claude-code/skill-writing.md) carries the craft — the two loads, the information hierarchy, steering, pruning, and the failure modes — and the `skill-creator` skill is the workflow over both. What follows is what a node skill needs on top of them.
...
- **Author against the whole contract, not a salient part.** What a node owes is the whole [node-skill contract](/software-factory/factory-operations.md#the-node-skill-contract); its most visible piece — a pinned interface, a headline item — is the public surface, not the whole of what's owed.
- **Read only what's needed.** Skip what's auto-loaded (project `CLAUDE.md`); `## Read first` ends in a `READ:` line; conditional reads stay at their point of use. Source the *what and when* — the routing and decisions the read already defines — from the source itself, and supply the *how*, the commands an action draws on.
- **Don't re-explain what `## Read first` taught.** Reference a concept from the standard; don't restate it — the agent has read it.
- **Two paddings a node skill invites.** The [no-op test](/standards/claude-code/skill-writing.md#pruning) cuts what the agent would do anyway, and two of its instances are node-shaped: the *downstream why*, explaining how a step serves a later phase the agent can't see ("commit so the human's push has something to publish"), and the *closed edge case*, guarding a scenario the factory design already settled ("a reboot won't lose the worktree"). Keep the conditions and escalations; drop the color.
...
- **Review nodes report, they don't fix.** A node that reviews another node's output stays read-only on it — the skill's prose forbids touching the work under review, and its `disallowed-tools` entry makes the harness refuse the write (see [factory-operations.md § Permissions](/software-factory/factory-operations.md#permissions)). Defects route back to the authoring node through the human's rework verdict, not the reviewer's hand.
...
- **Leave a deferred check out of the review entirely.** Where a check is deliberately settled once at the end of the traverse rather than at each node — the semantic judgment cache is the one in play ([factory-operations.md § the node-skill contract](/software-factory/factory-operations.md#the-node-skill-contract)) — a review skill neither arms it nor names it. Its declarations are outside the reviewer's jurisdiction and its red state mid-traverse is expected, so a finding against it is dismissed on arrival.
...
- **Permissions are the session's, not the skill's.** Auto mode judges each call, and subagent permissions are consciously wide (see [factory-operations.md § Permissions](/software-factory/factory-operations.md#permissions)) — a skill does not enumerate the permissions it needs. The exception is a role boundary the harness must enforce rather than merely be asked for: a reviewer's read-only stance is stated in the skill's prose *and* nailed down by `disallowed-tools` ([skill-conventions.md — Optional fields](/standards/claude-code/skill-conventions.md#optional-fields)).
- **An AFK skill escalates, it doesn't ask.** It runs in a subagent with no human attached, so when it needs a human call it ends with `ESCALATE:` — the issue overwatch bubbles it up. An interview skill is the reverse: the user is at the terminal, so it interviews freely in prose and gates on the answers.
...
- **Close with the standard report line.** One ` · `-delimited line: the handle `<repo>#<N>` · `phase: <node>`. A committing node appends `commit <sha> · check green · unpushed`; a node whose real output landed on GitHub appends a pointer only (`findings on PR #<n>`, `brief in issue`), never a re-paste — the terminal points, GitHub holds the detail. An AFK skill prefixes its terminal token per the subagent return contract: the final message begins at character one with exactly `DONE: <one-line outcome>` or `ESCALATE: <one-line reason>`, any detail below the line — semantics in [factory-operations.md § Engagement](/software-factory/factory-operations.md#engagement), where a non-matching final message reads as ESCALATE. An `ESCALATE:` carries a prose reason, the only record of the block. An inline skill omits the token and closes with the same line.
...
- **AFK skills don't gate on approval.** No mid-work "wait for the user" steps — the agent runs to its terminal line and escalates on exceptions; human review lives at the dedicated review nodes. An interview skill does gate — that's the line between the two.
