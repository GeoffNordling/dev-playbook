...
**Readiness.** An issue overwatch may launch on any **unblocked** issue — every issue in its blocked-by set closed (see [issue-conventions § Relationships](/standards/tracking/issues.md)) — with intake as its first HITL node when the issue is untriaged. Crossing into an implementation node requires more: the issue must be a **leaf** (epics never dispatch) with a brief-complete body per the [tracking standard](/standards/tracking/issues.md). An issue that fails that bar escalates to the human, whose call it is whether it goes back through intake. Blocked is a derived state GitHub surfaces in the Issues tab and Projects, not a label.
...
**Subagent permissions are consciously wide.** Subagent-level tool permissions are otherwise out of scope for this model: subagents run under auto mode with wide permissions, accepted deliberately. The one exception is the reviewer read-only guarantee — a reviewer reports findings, never rewrites the work under review — which the review skills carry in their `disallowed-tools` frontmatter, denying them the Edit/Write family everywhere but the `/tmp` carve-out each one stages its findings in before posting them.
...
- **HITL** — the issue overwatch runs the node itself, so the body may gate on interviews and approvals — asked in prose at the terminal — and the node closes with a plain report.
