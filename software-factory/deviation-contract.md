...
3. Does the fix contradict a decision recorded on the issue, PR, or map?
...
limiter tripped, and two or three fix options with a recommendation — to the
PR if one exists, otherwise to the issue — always the most current of the
two. The user's reply is the ruling: given as a comment, or spoken in the
terminal and transcribed onto the issue or PR by the session, so it is
always recorded. The brief body itself is never the place — it is frozen,
...
rework lap, with the PR live, the builder appends to the section directly.
