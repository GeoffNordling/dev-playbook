...
- **A node stages scratch outside the repository.** Intermediate text — a comment body, a PR description — goes to `/tmp/<kind>-<issue>.md` with the Write tool, then passes to `gh` with `--body-file`. The worktree holds the deliverable and nothing else.
