...
Its base is the general runbook machinery, equally unrestated: [runbook-conventions.md](/standards/harness/runbook-conventions.md) binds the format a skill or agent definition takes, the [`/writing-for-agents`](/dotfiles/dot-claude/skills/writing-for-agents/SKILL.md) skill carries the craft both answer to, and the `runbook-creator` skill is the workflow over both. What follows is what a node needs on top of them.
...
- **Paddings a node body invites.** The [no-op test](/dotfiles/dot-claude/skills/writing-for-agents/SKILL.md#pruning) cuts what the agent would do anyway. Node-shaped instances: explaining how a step serves a later phase the agent can't see ("commit so the later PR has content to carry"), and guarding a scenario the factory design settled ("a reboot won't lose the worktree"). Keep the conditions and escalations; drop the color.
