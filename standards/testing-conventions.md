...
- **Requirement markers.** Every test is marked with the requirement it covers: `@pytest.mark.req("req~area.name~1")`. This enables traceability verification via `pytest-sdd`. The marker value is the OFT spec item ID of the `req` item the test covers; see [sdd-standards/oft.md — Specification item ID](~/workspace/dev-playbook/sdd-standards/oft.md#specification-item-id) for the format.
