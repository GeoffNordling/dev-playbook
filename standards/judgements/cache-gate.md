...
description: The deterministic pytest gate — green iff every judgement's exact content is already judged-and-passed
---

# The Cache Gate

Every judgement is a **pytest** whose body does only a deterministic,
...
def test_errors_exhaustive():
    assert_judgement_cached("errors-exhaustive")
```

`assert_judgement_cached(id)` resolves the root
([declarations.md — Config and root resolution](/standards/judgements/declarations.md#config-and-root-resolution)),
keys the named judgement
([declarations.md — What a judgement is](/standards/judgements/declarations.md#what-a-judgement-is)),
and asks the seen-set whether that exact content has been
judged-and-passed:

- **cache hit → the test passes.** The judgement's content has already been
  ruled true by the judge.
- **cache miss → the test fails** with `judgement '<id>': cache miss`. The
  content is new or changed and needs the `run-judgements` skill to run it.
- an unknown `id`, or an unreadable evidence file, surfaces as a loud test
  error, never a silent pass.

A repo can instead cover **all** its judgements with one parametrized test,
...
So `pytest` is a fast gate: green means every judgement's current content is
already cached as passing. Filling the cache — running the judge on the
misses and recording the passes — is the job of the `run-judgements` skill,
...
judges through the harness interactively. A judgement whose judge returns
...
The cache miss is only remediable by `run-judgements` at the main loop, but
subagents are told to run `make check` regularly. So the gate is two-tier,
keyed off one environment variable, `SKIP_JUDGEMENTS`, read inside
`assert_judgement_cached` itself:

| Invocation | `SKIP_JUDGEMENTS` | The gate |
|---|---|---|
| `make check`, `make test` | `1` (the Makefile default, exported) | **skipped** |
| `make check-judgements` | `0` (`$(MAKE) check SKIP_JUDGEMENTS=0`) | **armed** |
| bare `uv run pytest` | unset | **armed** (fail-safe) |

When `SKIP_JUDGEMENTS` is exactly `1`, the helper **skips** each case with a
...
those recipes. `make check-judgements` is the
[pre-push hook](/standards/build/canonical/.pre-commit-config.yaml)'s entry, so
a miss blocks the push; the human or main-loop agent then runs `run-judgements`
