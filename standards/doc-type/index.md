...
- [Workstream Conventions](/standards/doc-type/workstream-conventions.md) — The form a Workstream's head file takes — its headings from one menu, Open holding questions only, each worklist item opening with its bold name, each stint entry in one form, targeting only rules of its own draft Standards, Stints in a leaf only, and every child reached by links from its parent
