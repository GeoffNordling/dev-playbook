...
| `Standard` | A normative conformance target: rules a repo, doc, or agent must follow, that a reviewer or linter could cite to reject work; lives under `standards/`, the one tree reserved for it (see [the Standard doc-type's encoding](/doc-types/standard/encoding.md#where-a-standard-lives)). |
