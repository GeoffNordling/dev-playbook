...
- [Type Registry](/standards/knowledge-organization/type-registry.md) — The document-type vocabulary a consumer adds to — the okf_types mapping in its root index, each entry's shape, the order of its keys, and no key that shadows a type
...
- [Cross-References](/standards/knowledge-organization/cross-references.md) — The cross-reference grammar — root-absolute Links in-bundle, workspace Citations across repos, the rootless forms, fragment anchors that match a heading's distinct slug, `#` headings only, and the link text that names the target
