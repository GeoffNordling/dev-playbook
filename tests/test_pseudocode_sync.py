"""The five contract-shape blocks match the end of the reference model's one.

The doc-type system's pseudocode is held whole in the reference model,
`doc-types/reference-model.md`: the base first, then one class per
doc-type. Each `contract-shape.md` holds its own class, and this test
fails when the five, joined in order, differ from the block after the
base. It parses nothing as Python: the pseudocode is a picture, not code. Delete this test when the
reference model is deleted; it fails with that instruction if the file
is missing.
"""
...
REFERENCE = ROOT / "doc-types/reference-model.md"
PAGES = [
...
def test_the_five_pages_concatenated_end_the_reference_models_block() -> None:
